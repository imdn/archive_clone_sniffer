...
def list_archive_contents(database, archive):
    ...
    try:
        if archive_only:
            arc = Archive(archive)
            empty_msg = "Archive '{}' seems to be empty".format(archive)
            header, records = arc.listArchiveContents()
        else:
            db = Database(database)
            empty_msg = "Database '{}' does not contain any records for archive '{}'".format(database, archive)
            header, records = db.listArchiveContents(archive)
            db.close()
        if len(records) > 0:
            rows = list(map(list, records)) # Convert tuple of tuples to list of lists
            data = [header]
            ...
            #table.inner_heading_row_border = False
            print(table.table)
            print("{} file(s) in archive '{}'".format(len(rows), archive))
            if archive_only:
                print("\nArchive SHA-1 checksum - {}".format(arc.sha1))
        else:
            print()
            print_banner(len(empty_msg), 'REPORT')
            print()
            print(empty_msg)

    except sqlite3.Error as e:
        print("ERROR! Could not list archive. Sqlite3 said: \"{}\"".format(e.message))
        db.close()
    except WindowsError as e:
        print("ERROR! Err. Message : \"{}\"".format(e))
        sys.exit(-1)
...
def init():
    ...
    if args.list:
        list_archive_contents(database, archive)
        return
